...
*/
__global__ void kernCopyPositionsToVBO(int num, glm::vec3 *pos, float *vbo, float s_scale) {
...
*/
__global__ void kernCopyRGBToVBO(int num, glm::vec3 *rgb, float *vbo, float s_scale) {
  int index = threadIdx.x + (blockIdx.x * blockDim.x);
...
void pointcloud::pointCloudToVBOGPU(int num, float *vbodptr_positions, float *vbodptr_rgb, float s_scale) {
	

	//Launching Kernels
	dim3 fullBlocksPerGrid((num + blockSize - 1) / blockSize);
	kernCopyPositionsToVBO<<<fullBlocksPerGrid, blockSize >>>(num, dev_pos, vbodptr_positions, s_scale);
	kernCopyRGBToVBO <<<fullBlocksPerGrid, blockSize >>>(num, dev_rgb, vbodptr_rgb, s_scale);
	utilityCore::checkCUDAErrorWithLine("copyPointCloudToVBO failed!");
	cudaDeviceSynchronize();
}

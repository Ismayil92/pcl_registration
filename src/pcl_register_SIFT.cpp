...
int main (int argc, char** argv)
{
  ...
  // Initialize ROS
  ros::init (argc, argv, "point_cloud_register");
  ros::NodeHandle nh;
  ros::Rate r(20);
  // Create a ROS subscriber for the input point cloud from the sensor
  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/camera/depth_registered/points", 1, cloud_cb);
  ros::Publisher pub = nh.advertise<sensor_msgs::PointCloud2> ("xtion/depth_registered/registered_points", 20);
  ...
  while(ros::ok())
  {
    ...
    if(cycle>1)
    {
        ...
        pcl::io::savePCDFileASCII("data_scene_filtered_SIFT.pcd", *scene_cloud_ptr);
        ...
        // NOTE: T_matrix coming from SVD. Rough_T_matrix_from_pose_est coming from Pose Estimation
      if(cycle<3)
      {
        std::cout << "Estimated Transform:" << std::endl << T_matrix << std::endl;
        pcl::transformPointCloud(*scene_cloud_ptr, *scene_cloud_transformed, T_matrix);
        pcl::io::savePCDFileASCII("data_init_trans_sift.pcd", *scene_cloud_transformed);
      }
        /*pcl::transformPointCloud(*scene_cloud_ptr, *scene_cloud_transformed_dl, Rough_T_matrix_from_pose_est.inverse());
        pcl::io::savePCDFileASCII("shekil_data_dl.pcd", *scene_cloud_transformed_dl);
      ...
      if(cycle<3)
      {
          ...
          pcl::VoxelGrid<pcl::PointXYZ> sor_main;
          sor_main.setInputCloud (ref_main_cloud_ptr);
          sor_main.setLeafSize (0.005, 0.005, 0.005);
          sor_main.filter (*ref_main_cloud_ptr);
